...
export default class PomodoroCalendarPlugin extends Plugin {
	...
	.pomodoro-animated-controls { position: absolute; top: 4px; right: 50%; transform: translateX(50%); display: flex; gap: 6px; flex-direction: row; z-index: 10; opacity: 0; pointer-events: none; transition: opacity 0.2s ease; }
...
.pomodoro-direction-rtl .pomodoro-animated-controls { right: auto; left: 50%; transform: translateX(-50%); }
	...
	.pomodoro-progress-trail { position: absolute; bottom: 4px; left: 12px; right: 12px; height: 12px; background: color-mix(in srgb, var(--interactive-accent) 15%, transparent); border-radius: 6px; overflow: hidden; z-index: 6; pointer-events: none; }
	.pomodoro-white-track { position: absolute; top: 0; left: 0; height: 100%; width: 0%; background: color-mix(in srgb, var(--interactive-accent) 80%, transparent); transition: width 0.3s ease; border-radius: 6px; pointer-events: none; }
	...
	.pomodoro-progress-text { position: absolute; top: 36px; left: 12px; right: 12px; display: flex; align-items: center; gap: 12px; font-family: var(--font-monospace, monospace); pointer-events: none; justify-content: flex-end; }
